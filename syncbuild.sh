...
elif [ ${BUILDID:0:11} == "LE.UM.4.3.1" ]; then
  TARGET=genericarmv8-vm-debug
elif [ ${BUILDID:0:11} == "LE.UM.5.3.2" ]; then
  TARGET=sdxlemur-mbb-nogplv3-debug
...
if [ ${BUILDID:0:11} == "LE.UM.4.3.1" ]; then
  export MACHINE=genericarmv8
  export DISTRO=qti-distro-debug
  source poky/qti-conf/set_bb_env.sh
  BUILD_CMD="bitbake qti-vm-image"
elif [ ${BUILDID:0:11} == "LE.UM.5.3.2" ]; then
  export MACHINE=sdxlemur
  export DISTRO=qti-distro-nogplv3-debug
  source poky/qti-conf/set_bb_env.sh
  BUILD_CMD="bitbake qti-mbb-image"
elif [ ${BUILDID:18:4} == "8x96" ] && [ ${BUILDID:0:6} == "LV.HB." ]; then
